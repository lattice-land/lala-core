...
// for parsing onnx and vnnlib file in neural network verification project.
inline VarEnv<standard_allocator> env_with(const char* onnx, const char* vnnlib){
  VarEnv<standard_allocator> env;
  auto onnx_formula = parse_onnx_str<standard_allocator>(onnx);
  auto f = parse_vnnlib_str<standard_allocator>(vnnlib, onnx_formula);
  EXPECT_TRUE(f);
  IDiagnostics diagnostics;
  if(f->is(F::Seq) && f->sig() == AND){
      for(int i = 0; i < f->seq().size(); ++i){
...
/** Initialize an environment with single float variable named `x` in the abstract domain typed `0.0`. */
inline VarEnv<standard_allocator> env_with_x() {
  return env_with("var float: x :: abstract(0.0);");
}

template<IKind kind, class L> 
void interpret_must_error(const char* onnx, const char* vnnlib, VarEnv<standard_allocator> env = VarEnv<standard_allocator>{}){
  static_assert(kind == IKind::TELL || L::is_abstract_universe);
  auto onnx_formula = parse_onnx<standard_allocator>(onnx);
  auto f = parse_vnnlib_str<standard_allocator>(vnnlib, onnx_formula);
  EXPECT_TRUE(f);
  IDiagnostics diagnostics; 
...
template<class L>
void both_interpret_must_error(const char* onnx, const char* vnnlib, VarEnv<standard_allocator> env = VarEnv<standard_allocator>{}){
  interpret_must_error<IKind::TELL, L>(onnx, vnnlib, env);
  interpret_must_error<IKind::ASK, L>(onnx, vnnlib, env);
}

template <IKind kind, bool ternarize_formula = false, class L>
void interpret_must_succeed(const char* onnx, const char* vnnlib, L& value, VarEnv<standard_allocator>& env, bool has_warning = false) {
  static_assert(kind == IKind::TELL || L::is_abstract_universe);
  auto onnx_formula = parse_onnx_str<standard_allocator>(onnx);
  auto f = parse_vnnlib_str<standard_allocator>(vnnlib, onnx_formula);
  EXPECT_TRUE(f);
  ...
  *f = normalize(*f);
  ...
  if(!res) {
    diagnostics.print();
    EXPECT_TRUE(false) << "The formula should be interpretable: " << onnx << ", " << vnnlib;
  }
  if(diagnostics.has_warning() && !has_warning) {
    diagnostics.print();
    EXPECT_TRUE(false) << "The formula generates a warning but should not: " << onnx << ", " << vnnlib;
  }
...
L create_and_interpret_and_type_and_tell(const char* onnx, const char* vnnlib, VarEnv<standard_allocator>& env, Typing&& typing, bool has_warning = false) {
  auto onnx_formula = parse_onnx<standard_allocator>(onnx);
  auto f = parse_vnnlib_str<standard_allocator>(vnnlib, onnx_formula);
  EXPECT_TRUE(f);
  if(ternarize_formula) {
...
L create_and_interpret_and_tell(const char* onnx, const char* vnnlib, VarEnv<standard_allocator>& env, bool has_warning = false) {
  return create_and_interpret_and_type_and_tell<L, ternarize_formula>(onnx, vnnlib, env, [](const F&){}, has_warning);
}

template <class L, bool ternarize_formula = false>
L create_and_interpret_and_tell(const char* onnx, const char* vnnlib, bool has_warning = false) {
  VarEnv<standard_allocator> env;
  return create_and_interpret_and_tell<L, ternarize_formula>(onnx, vnnlib, env, has_warning);
}

template <IKind kind, class L>
void expect_interpret_equal_to(const char* onnx, const char* vnnlib, const L& expect, VarEnv<standard_allocator> env = VarEnv<standard_allocator>{}, bool has_warning = false) {
  L value{L::top()};
  interpret_must_succeed<kind>(onnx, vnnlib, value, env, has_warning);
  EXPECT_DOUBLE_EQ(value.lb(), expect.lb());
  EXPECT_DOUBLE_EQ(value.ub(), expect.ub());
}
...
template <class L>
void expect_both_interpret_equal_to(const char* onnx, const char* vnnlib, const L& expect, const VarEnv<standard_allocator>& env = VarEnv<standard_allocator>{}, bool has_warning = false) {
  expect_interpret_equal_to<IKind::TELL>(onnx, vnnlib, expect, env, has_warning);
  expect_interpret_equal_to<IKind::ASK>(onnx, vnnlib, expect, env, has_warning);
}

template <class L, bool ternarize_formula = false>
bool interpret_and_ask(const char* onnx, const char* vnnlib, L& value, VarEnv<standard_allocator>& env, bool has_warning = false) {
  auto onnx_formula = parse_onnx<standard_allocator>(onnx);
  auto f = parse_vnnlib_str<standard_allocator>(vnnlib, onnx_formula);
  EXPECT_TRUE(f);
  if(ternarize_formula) {
  ...
  *f = normalize(*f);
  ...
  if(!ginterpret_in<IKind::ASK, true>(value, *f, env, ask, diagnostics)) {
    diagnostics.print();
    EXPECT_TRUE(false) << "The formula should be (ask-)interpretable: " << onnx << ", " << vnnlib;
  }
  EXPECT_EQ(diagnostics.has_warning(), has_warning);
...
void join_one_test(const A& a, const A& b, const A& expect, bool has_changed_expect, bool test_tell = true) {
  EXPECT_DOUBLE_EQ(fjoin(a,b).lb(), expect.lb()) << "join(" << a << ", " << b << ")";
  EXPECT_DOUBLE_EQ(fjoin(a,b).ub(), expect.ub()) << "join(" << a << ", " << b << ")";
  if(test_tell) {
    A c(a);
    EXPECT_EQ(c.join(b), has_changed_expect) << a << ".join(" << b << ") == " << expect;
    EXPECT_DOUBLE_EQ(c.lb(), expect.lb()) << a << ".join(" << b << ")";
    EXPECT_DOUBLE_EQ(c.ub(), expect.ub()) << a << ".join(" << b << ")";
  }
}

template <class A>
void meet_one_test(const A& a, const A& b, const A& expect, bool has_changed_expect, bool test_tell = true) {
  EXPECT_DOUBLE_EQ(fmeet(a, b).lb(), expect.lb()) << "meet(" << a << ", " << b << ")";
  EXPECT_DOUBLE_EQ(fmeet(a, b).ub(), expect.ub()) << "meet(" << a << ", " << b << ")";
  if(test_tell) {
    A c(a);
    EXPECT_EQ(c.meet(b), has_changed_expect) << c << ".meet(" << b << ")";
    EXPECT_DOUBLE_EQ(c.lb(), expect.lb()) << c << ".meet(" << b << ")";
    EXPECT_DOUBLE_EQ(c.ub(), expect.ub()) << c << ".meet(" << b << ")";
  }
}
...
void generic_abs_test() {
  ...
  r.project(ABS, A::top());
  // EXPECT_EQ(r, a);
  EXPECT_DOUBLE_EQ(r.lb(), a.lb());
  EXPECT_DOUBLE_EQ(r.ub(), a.ub());
}

template <class A, class R = A>
void generic_binary_fun_test(Sig fun, const A& a) {
  battery::print(fun);
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::bot(), A::bot()).lb()), R::bot().lb());
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::top(), A::top()).ub()), R::top().ub());
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::top(), A::bot()).lb()), R::bot().lb());
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::bot(), A::top()).ub()), R::bot().ub());

  if(!is_division(fun)) {
    EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::top(), a).ub()), R::top().ub()) << A::top() << " " << string_of_sig(fun) << " " << a;
    EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, a, A::top()).ub()), R::top().ub()) << a  << " " << string_of_sig(fun) << " " << A::top();
  }
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, A::bot(), a).lb()), R::bot().lb());
  EXPECT_DOUBLE_EQ((project_fun<A, R>(fun, a, A::bot()).lb()), R::bot().lb());
}
...
template <class L, bool ternarize_formula = false>
void check_interpret_idempotence(const char* onnx, const char* vnnlib) {
  using F = TFormula<standard_allocator>;
  VarEnv<standard_allocator> env1, env2;
  L value1 = create_and_interpret_and_tell<L, ternarize_formula>(onnx, vnnlib, env1);
