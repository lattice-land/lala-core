...
struct PreBDec {
  ...
  template<class F>
  CUDA static iresult<F> interpret_tell(const F& f) { return dual_type::interpret_ask(f); }

  template<class F>
  CUDA static iresult<F> interpret_ask(const F& f) { return dual_type::interpret_tell(f); }
